    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="pull-right">
                <div class="pull-right"><a href="{{ route('empresas.create') }}" type="button" class="btn btn-success"><i class="fa fa-plus"></i> Novo {{substr_replace("Médicos", "", -1)}}</a></div>
            </div>
        </div>
    </div>
</div>
...
@endif
                        ...
                        @if(is_null($param))
<th><a href="{{Request::fullUrl()}}{{$signal}}order=crm" class="white-text templatemo-sort-by">CRM <span class="fa fa-caret-{{$caret}}"></span></a></th>
@else
@if(strpos($param,'desc') !== false)
<th><a href="{{str_replace(str_replace(' ','%20',$param),'crm',Request::fullUrl())}}" class="white-text templatemo-sort-by @if(strpos($param,'crm') !== false)active @endif">CRM <span class="fa fa-caret-{{$caret}}"></span></a></th>
@else
<th><a href="{{str_replace('order='.$param,'order=crm',Request::fullUrl())}} @if($param == 'crm')desc @endif" class="white-text templatemo-sort-by @if(strpos($param,'crm') !== false)active @endif">CRM <span class="fa fa-caret-{{$caret}}"></span></a></th>
@endif
@endif
...
@endif
                        ...
                        @if(is_null($param))
<th><a href="{{Request::fullUrl()}}{{$signal}}order=telefone" class="white-text templatemo-sort-by">Telefone <span class="fa fa-caret-{{$caret}}"></span></a></th>
@else
@if(strpos($param,'desc') !== false)
<th><a href="{{str_replace(str_replace(' ','%20',$param),'telefone',Request::fullUrl())}}" class="white-text templatemo-sort-by @if(strpos($param,'telefone') !== false)active @endif">Telefone <span class="fa fa-caret-{{$caret}}"></span></a></th>
@else
<th><a href="{{str_replace('order='.$param,'order=telefone',Request::fullUrl())}} @if($param == 'telefone')desc @endif" class="white-text templatemo-sort-by @if(strpos($param,'telefone') !== false)active @endif">Telefone <span class="fa fa-caret-{{$caret}}"></span></a></th>
@endif
@endif
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="small" align="center" alt="Visualizar Assinatura Digital">
                        @if($empresa->anexo)
                        <a href="{{ "storage/empresas/$empresa->id/$empresa->anexo", $empresa->anexo }}" target="_blank">
