    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="pull-right">
                <div class="pull-right"><a href="{{ route('pacientes.create') }}" type="button" class="btn btn-success"><i class="fa fa-plus"></i> Novo {{substr_replace("Pacientes", "", -1)}}</a></div>
            </div>
        </div>
    </div>
</div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="small" align="center" alt="Deletar Paciente">
                        <a onclick="confirm_delete('{{ route('pacientes.destroy', $paciente->id) }}')" href="javascript:;" data-toggle="modal" data-target="#confirm_delete">
...
<div class="pagination-wrap">
    <p class="text_pagination pull-left">Exibindo do <strong>{{$pacientes->firstItem()}}</strong> ao <strong>{{$pacientes->lastItem()}}</strong> de um total de <strong>{{$pacientes->total()}}</strong> registros</p>
    {!! $pacientes->render() !!}
</div>
@else
<div class="templatemo-content-widget no-padding">
