...
<div class="templatemo-content-widget white-bg">
    <h2 class="margin-bottom-10">
        ...
        Cadastrar {{substr_replace("Contas", "", -1)}} @if($tipo == '0') a Pagar @else a Receber @endif
        @else
        Editar {{substr_replace("Contas", "", -1)}} @if($tipo == '0') a Pagar @else a Receber @endif #{{$contum->id}}
        @endif
    </h2>
    ...
    <div id="formulario_0" class="formulario">
        <div class="row form-group row-multiple">
            <div class="col-md-3 col-sm-12 col-xs-12">
                ...
                {!! Form::date('date[0]', $contum->date, ['class' => 'form-control date','required' => 'true','readonly' => true]) !!}
            </div>
            <div class="col-md-3 col-sm-12 col-xs-12">
                {!! Html::decode(Form::label('tipo_pagamento', 'Tipo de Pagamento <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                @if($tipo == "0")
                {!! Form::select("tipo_pagamento[0]", ['Crédito' => 'Crédito', 'Débito' => 'Débito', 'Boleto' => 'Boleto', 'Cheque' => 'Cheque', 'Nota Fiscal' => 'Nota Fiscal', 'Recibo' => 'Recibo', 'Espécie' => 'Espécie', 'Transferência' => 'Transferência'], $contum->tipo_pagamento, ['id' => 'tipo_pagamento_0', 'class' => 'form-control select2-search tipo_pagamento', 'required' => 'true']) !!}
                @else
            ...
            <div class="col-md-3 col-sm-12 col-xs-12">
                {!! Html::decode(Form::label('qtd_parcelas', 'Qtd Parcelas <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                {!! Form::number('qtd_parcelas[0]', $contum->qtd_parcelas, ['class' => 'form-control qtd_parcelas','onKeyPress' => 'validar_numero(event)', 'required' => 'true']) !!}
            </div>
            <div class="col-md-3 col-sm-12 col-xs-12">
                {!! Html::decode(Form::label('cartao', 'Cartão <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                {!! Form::select("cartao[0]", ["Visa" => "Visa", "Mastercard" => "Mastercard", "American Express" => "American Express", "Elo" => "Elo", "Discover Network" => "Discover Network"], $contum->cartao, ['id' => 'cartao', 'class' => 'form-control select2-search cartao', 'required' => 'true']) !!}
            </div>
        </div>
        <div class="row form-group">
            ...
            <div class="col-md-2 col-sm-12 col-xs-12">
                {!! Html::decode(Form::label('desconto', 'Desconto <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                @if($method == 'post') {!! Form::text('desconto', NULL, ['class' => 'form-control desconto','onKeyDown' => 'Formata(this,20,event,2)', 'required' => 'true']) !!} @else {!! Form::text('desconto', number_format($contum->desconto,2,',','.'), ['class' => 'form-control', 'onKeyDown' => 'Formata(this,20,event,2)', 'required' => 'true']) !!} @endif
            </div>
            <div class="col-md-2 col-sm-12 col-xs-12">
                {!! Html::decode(Form::label('pago', 'Pago <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                @if($method == 'post') {!! Form::text('pago[0]', NULL, ['class' => 'form-control pago','onKeyDown' => 'Formata(this,20,event,2)', 'required' => 'true']) !!} @else {!! Form::text('pago[0]', number_format($contum->pago,2,',','.'), ['class' => 'form-control', 'onKeyDown' => 'Formata(this,20,event,2)', 'required' => 'true']) !!} @endif
            </div>
        </div>
        <div class="row form-group">
        ...
        <div class="row linha_parcela">
            @foreach($contum->parcelas()->get() as $index => $parcela)
            <div class="col-md-3">
                {!! Html::decode(Form::label('vencimento', ($index+1)."º Vencimento <span class='obrigatorio'>*</span>", ['class' => 'control-label'])) !!}
                {!! Form::text("vencimento[0][]", @date_format(date_create_from_format('Y-m-d', $parcela->vencimento), 'd/m/Y'), ['class' => 'form-control datepicker', 'required' => 'true']) !!}
            </div>
            <div class="col-md-3">
                {!! Html::decode(Form::label('parcela_valor', ($index+1)."º Valor <span class='obrigatorio'>*</span>", ['class' => 'control-label'])) !!}
                {!! Form::text("parcela_valor[0][]", number_format($parcela->valor,2,',','.'), ['class' => 'form-control parcela_valor','onKeyDown' => 'Formata(this,20,event,2)','onKeyPress' => 'validar_numero(event)', 'required' => 'true']) !!}
            </div>
            @endforeach
        </div>
        @endif
    ...
    @foreach($contum->contas()->get() as $key => $conta)
    {!! Form::hidden("ids[".($key+1)."]", $conta->id) !!}
    <div id="formulario_{{$key+1}}" class="formulario">
        <hr id="hr_{{($key+1)}}" />
        <div id="formulario_{{($key+1)}}" class="formulario formulario-multiple">
            <a href="javascript:;" class="fechar-formulario" data-id-formulario="{{($key+1)}}"><i class="fa fa-remove"></i></a>
        </div>
        <div class="row form-group row-multiple">
            <div class="col-md-3 col-sm-12 col-xs-12">
                {!! Html::decode(Form::label('data', 'Data de Lançamento <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                @if($method == 'post') {!! Form::date("date[".($key+1)."]", NULL, ['class' => 'form-control date','required' => 'true','readonly' => true]) !!} @else {!! Form::date("date[".($key+1)."]", $conta->date, ['class' => 'form-control date','required' => 'true','readonly' => true]) !!} @endif
            </div>
            <div class="col-md-3 col-sm-12 col-xs-12">
                {!! Html::decode(Form::label('tipo_pagamento', 'Tipo de Pagamento <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                @if($tipo == "0")
                {!! Form::select("tipo_pagamento[".($key+1)."]", ['Crédito' => 'Crédito', 'Débito' => 'Débito', 'Boleto' => 'Boleto', 'Cheque' => 'Cheque', 'Nota Fiscal' => 'Nota Fiscal', 'Recibo' => 'Recibo', 'Espécie' => 'Espécie', 'Transferência' => 'Transferência'], $conta->tipo_pagamento, ['id' => 'tipo_pagamento_0', 'class' => 'form-control select2-search tipo_pagamento', 'required' => 'true']) !!}
                @else
            ...
            <div class="col-md-3 col-sm-12 col-xs-12 @if(!in_array($conta->tipo_pagamento, ['Crédito','Boleto','Cheque Pré-Datado','Cheque'])) hide @endif">
                {!! Html::decode(Form::label('qtd_parcelas', 'Qtd de Parcelas <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                {!! Form::text("qtd_parcelas[".($key+1)."]", $conta->qtd_parcelas, ['class' => 'form-control qtd_parcelas','onKeyPress' => 'validar_numero(event)', 'required' => 'true', 'maxlength' => '11']) !!}
            </div>
            <div class="col-md-3 col-sm-12 col-xs-12">
                {!! Html::decode(Form::label('cartao', 'Cartão <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                @if($method == 'post') {!! Form::select("cartao[".($key+1)."]", ["Visa" => "Visa", "Mastercard" => "Mastercard", "American Express" => "American Express", "Elo" => "Elo", "Discover Network" => "Discover Network"], NULL, ['id' => 'cartao', 'class' => 'form-control select2-search cartao', 'required' => 'true']) !!} @else {!! Form::select("cartao[".($key+1)."]", ["Visa" => "Visa", "Mastercard" => "Mastercard", "American Express" => "American Express", "Elo" => "Elo", "Discover Network" => "Discover Network"], $conta->cartao, ['id' => 'cartao', 'class' => 'form-control select2-search cartao', 'required' => 'true']) !!} @endif
            </div>
        </div>
        <div class='row'>
            <div class='col-md-2 col-sm-6 col-xs-6 margin-bottom-15'>
                {!! Html::decode(Form::label('pago', 'Pago <span class="obrigatorio">*</span>', ['class' => 'control-label'])) !!}
                {!! Form::text("pago[".($key+1)."]", number_format($conta->pago,2,',','.'), ['class' => 'form-control pago','onKeyDown' => 'Formata(this,20,event,2)','onKeyPress' => 'validar_numero(event)', 'required' => 'true']) !!}
            </div>
        </div>
        @if(in_array($conta->tipo_pagamento, ["Boleto","Cheque Pré-Datado"]))
        <div class="row linha_parcela">
            @foreach($conta->parcelas()->get() as $index => $parcela)
            <div class="col-md-3">
                {!! Html::decode(Form::label('vencimento', ($index+1)."º Vencimento <span class='obrigatorio'>*</span>", ['class' => 'control-label'])) !!}
                {!! Form::text("vencimento[".($key+1)."][]", @date_format(date_create_from_format('Y-m-d', $parcela->vencimento), 'd/m/Y'), ['class' => 'form-control datepicker', 'required' => 'true']) !!}
            </div>
            <div class="col-md-3">
                {!! Html::decode(Form::label('parcela_valor', ($index+1)."º Valor <span class='obrigatorio'>*</span>", ['class' => 'control-label'])) !!}
                {!! Form::text("parcela_valor[".($key+1)."][]", number_format($parcela->valor,2,',','.'), ['class' => 'form-control parcela_valor','onKeyDown' => 'Formata(this,20,event,2)','onKeyPress' => 'validar_numero(event)', 'required' => 'true']) !!}
            </div>
            @endforeach
        </div>
        @endif
...
<div class="templatemo-content-widget no-padding">
    <div class="panel panel-default table-responsive">
        <table class="table table-striped table-bordered templatemo-user-table">
            ...
            <tbody>
                @foreach($contum->parcelas()->get() as $parcela)
                <tr>
                    ...
                    <td>{{@date_format(date_create_from_format('Y-m-d', $parcela->conferencia), 'd/m/Y')}}</td>
                    ...
                    <td class="small" align="center" title="Pago">
                        @if($parcela->pago)
                        <a onclick="confirm_pagar('{{ route('parcelas.pagar', ['ids' => [$parcela->id], 'tipo' => $tipo, 'desfazer' => true]) }}')" href="javascript:;" data-toggle="modal" data-target="#confirm_pagar"><i class="fa fa-check-square"></i></a>
                        @else
                        <a onclick="confirm_pagar('{{ route('parcelas.pagar', ['ids' => [$parcela->id], 'tipo' => $tipo]) }}')" href="javascript:;" data-toggle="modal" data-target="#confirm_pagar"><i class="fa fa-square-o"></i></a>
                        @endif
                    </td>
                </tr>
                @endforeach
                @foreach($contum->contas()->get() as $conta)
                @foreach($conta->parcelas()->get() as $parcela)
                <tr>
                    <td>{{@date_format(date_create_from_format('Y-m-d', $parcela->lancamento), 'd/m/Y')}}</td>
                    <td>{{@date_format(date_create_from_format('Y-m-d', $parcela->vencimento), 'd/m/Y')}}</td>
                    <td>{{@date_format(date_create_from_format('Y-m-d', $parcela->conferencia), 'd/m/Y')}}</td>
